...
function sortByManualOrderThenDateDesc(items) {
  return [...(items || [])].sort((a, b) => {
    const pa = normalizeSortOrder(a.sortOrder);
    const pb = normalizeSortOrder(b.sortOrder);
...
function projectCard(p, opts = {}) {
  const showSortOrder = !!opts.showSortOrder;
  ...
  if (showSortOrder) {
    meta.appendChild(
      el("span", "px-2 py-1 rounded-full bg-white/5 border border-white/10", `Manual sort order: ${normalizeSortOrder(p.sortOrder)}`)
    );
  }
  if (p.date) {
...
function educationCard(e, opts = {}) {
  ...
  // if (showSortOrder) {
  //   metaRow.appendChild(
  //     el("span", "px-2 py-1 rounded-full bg-white/5 border border-white/10", `Manual sort order: ${normalizeSortOrder(e.sortOrder)}`)
  //   );
  // }
...
function experienceCard(x, opts = {}) {
  ...
  // if (showSortOrder) {
  //   metaRow.appendChild(
  //     el("span", "px-2 py-1 rounded-full bg-white/5 border border-white/10", `Manual sort order: ${normalizeSortOrder(x.sortOrder)}`)
  //   );
  // }
