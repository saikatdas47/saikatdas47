(async function () {
  const GRID_ID = "publicationsGrid";
  const SECTION_ID = "publicationsSection";
  const EMPTY_ID = "emptyPublications";
  ...
  function sortByDate(items, mode) {
    ...
    return list.sort((a, b) => {
      ...
      return mode === "oldest"
        ? dateA - dateB
        : dateB - dateA; // newest default
    });
  }

  try {
    const items = await loadJSON("data/publications.json");
    ...
    function paint() {
      ...
      grid.innerHTML = "";
      sorted.forEach((p) => grid.appendChild(publicationCard(p)));
    }
